...
def vister_index(user_id):
   ...
   # user_id = current_user.id
        ...
        return redirect(url_for('user.resume',user_id=user_id))
...
@vister_required
@user.route('/vister/<user_id>/updateresume',methods=['GET','POST'])
def updateresume(user_id):
    print("hello")
    resume = Resume.query.filter_by(user_id=user_id).first()
    print(type(resume.phone))
    form = ResumeForm(obj=resume)
    if form.validate_on_submit():
       form.update_resume(resume)
       flash("更新成功","success")
       return redirect(url_for('user.vister_index',user_id=user_id))
    return render_template("user/update_resume.html",form=form,resume=resume)
@vister_required
@user.route('/vister/<user_id>/deliverlist/')
def deliverlist(user_id):
    delivers = Deliver.query.filter_by(user_id=user_id).all()
    return render_template("user/vister_deliverlist.html",delivers=delivers)
@hr_required
...
@hr_required
@user.route('/hr/<user_id>/delivercheck',methods=['GET'])
def delivercheck(user_id):
    company_id = User.query.get(user_id).companyid
    delivers = Delivers.query.filter_by(company_id=company_id)
    return render_template('user/hr_delivercheck.html',delivers=delivers)
